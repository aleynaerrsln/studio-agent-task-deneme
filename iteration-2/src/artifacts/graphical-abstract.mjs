...
export const spec = {
  ...
  stale_rules: [
    ...
    'panel primary_number substring doğrulaması başarısız olursa artifact invalid kabul edilmeli',
  ],
};
...
const lintRules = [
  ...
  {
    id: 'primary-number-traceable',
    check: (a) => {
      const kbNorm = normalizeForTrace(a._kb_raw_text ?? '');
      if (!kbNorm) return { error: 'knowledge-base ham metni sağlanmadı — trace yapılamadı' };
      const misses = [];
      for (const p of a.payload.panels ?? []) {
        const pn = String(p.primary_number ?? '');
        if (!pn.trim()) continue;
        const needle = normalizeForTrace(pn);
        if (!kbNorm.includes(needle))
          misses.push(`${p.role}: "${pn}"`);
      }
      return (
        misses.length === 0 || {
          error: `primary_number knowledge-base'de substring olarak bulunamadı: ${misses.join('; ')}`,
        }
      );
    },
  },
...
export function compile(wiki) {
  ...
  const payload = {
    ...
    panels,
    footer: {
      key_stats: buildKeyStats({ dependent, independent }),
      disclaimer:
...
function buildKeyStats({ dependent, independent }) {
  const stats = [];
  if (dependent) {
    stats.push('p < 0.001');
    stats.push('r = -0.613 büyük etki');
  }
  if (independent) stats.push('Cohen\'s d = 4.186 (cov1)');
  return stats;
}
