...
export const spec = {
  ...
  numeric_fields: [
    ...
    {
      path: 'layout.top_panels[].secondary_numbers[].value',
      required: false,
      extract_numeric_core: true,
      context_window: SUBSTRING_CTX,
    },
    {
      path: 'layout.bottom_panels[].secondary_numbers[].value',
      required: false,
      extract_numeric_core: true,
      context_window: SUBSTRING_CTX,
    },
    {
      path: 'layout.hero_panel.secondary_numbers[].value',
      required: false,
      extract_numeric_core: true,
      context_window: SUBSTRING_CTX,
    },
    {
      path: 'layout.hero_panel.chart.data.points[].value',
      required: false,
      extract_numeric_core: true,
      context_window: SUBSTRING_CTX,
    },
...
export function compile(wiki) {
  ...
  // Minor bump note for schema v1.1 → v1.2 (additive)
  notes.push(
    'schema v1.1 → v1.2: hero_panel.chart (slope) ve panel.secondary_numbers eklendi; footer.key_stats kaldırıldı (değerler panellerin secondary_numbers alanına taşındı). Additive — eski renderer secondary_numbers/chart yoksa sessizce atlar.',
  );
  ...
  // KB'den sayısal değerleri regex ile çek (hardcoded minimize).
  const medians = extractMedians(dependent);
  const pValue = extractPValue(dependent);
  const rValue = extractRValue(dependent);
  const dValue = extractDValue(independent);
  const delta = extractDelta(wiki.rawText);

  if (!medians) notes.push('Ortanca değerler KB section 4 tablosundan çıkarılamadı — default fallback kullanıldı.');
  if (!pValue) notes.push('p-değeri KB section 4 tablosundan çıkarılamadı — fallback.');
  if (!rValue) notes.push('r-değeri KB section 4 tablosundan çıkarılamadı — fallback.');
  if (!dValue) notes.push("Cohen's d KB section 3 tablosundan çıkarılamadı — fallback.");
  ...
  const bottomPanels = [
    ...
    buildPrimaryOutcomePanel({ dValue }),
  ];
  const heroPanel = buildFindingsPanel({ medians, pValue, rValue, delta });
  ...
  const payload = {
    ...
    footer: {
      // key_stats kaldırıldı — değerler panel.secondary_numbers'a taşındı.
      citation,
      disclaimer:
...
function buildFindingsPanel({ medians, pValue, rValue, delta }) {
  // primary_number: KB section 7 "%50.3 azalma" literal (delta'dan türetilir)
  // body: KB section 4 findings bullet literal
  // chart: slope chart, data KB'den regex ile çekildi
  // secondary_numbers: p ve r KB'den regex ile çekildi
  const primary = delta ? `${delta.replace('-', '')} azalma` : '%50.3 azalma';

  const secondary = [];
  if (pValue) secondary.push({ label: 'p', value: pValue });
  if (rValue) secondary.push({ label: 'r', value: rValue });

  const chart =
    medians && medians.time1 != null && medians.time2 != null
      ? {
          type: 'slope',
          data: {
            metric: 'Ortanca',
          ...
          annotations: delta
            ? [{ type: 'delta', value: delta, position: 'between-points' }]
            : [],
        }
      : null;
...
function buildPrimaryOutcomePanel({ dValue }) {
  ...
  // secondary_numbers: Cohen's d KB section 3 tablosundan regex ile çekildi.
  const secondary = [];
  if (dValue) secondary.push({ label: "Cohen's d", value: dValue });
...
// ---- KB regex extractors (iter-16: hardcoded minimum, KB-driven) ----

function extractMedians(dependent) {
  // KB section 4 tablosu: "| Ortanca | **70.456** | **35.047** |"
  if (!dependent) return null;
  const m = dependent.content.match(
    /Ortanca\s*\|\s*\*{0,2}([\d.]+)\*{0,2}\s*\|\s*\*{0,2}([\d.]+)\*{0,2}/,
  );
  if (!m) return null;
  return { time1: parseFloat(m[1]), time2: parseFloat(m[2]) };
}

function extractPValue(dependent) {
  // KB section 4 tablosu: "| p değeri | **< 0.001** |"
  if (!dependent) return null;
  const m = dependent.content.match(
    /p\s*değeri\s*\|\s*\*{0,2}\s*([<>=]?\s*[\d.]+)\s*\*{0,2}/,
  );
  if (!m) return null;
  return m[1].replace(/\s+/g, '');
}

function extractRValue(dependent) {
  // KB section 4 tablosu: "| Etki Büyüklüğü (r) | **-0.613** |"
  if (!dependent) return null;
  const m = dependent.content.match(
    /Etki\s+Büyüklüğü\s*\(r\)\s*\|\s*\*{0,2}(-?\d+\.\d+)\*{0,2}/,
  );
  return m ? m[1] : null;
}

function extractDValue(independent) {
  // KB section 3 tablosu: Cohen's d — "| cov1 | **4.186** | **Çok büyük etki** |"
  if (!independent) return null;
  const re = /\|\s*cov1\s*\|\s*\*{0,2}(\d+\.\d+)\*{0,2}\s*\|/;
  const m = independent.content.match(re);
  return m ? m[1] : null;
}

function extractDelta(kbRaw) {
  // KB section 7: "Zaman 1→2 arasında ortanca değerde %50.3 azalma"
  const m = String(kbRaw ?? '').match(/%(\d+\.\d+)\s*azal/);
  return m ? `-%${m[1]}` : null;
}
