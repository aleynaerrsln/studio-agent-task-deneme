...
export async function compile(wiki, options = {}) {
  // clinical-summary şu an mode-agnostic (iter-18 kapsamı dışında);
  // async imza CLI tarafında uyumluluk için korunur.
  void options;
  return compileSync(wiki);
}

function compileSync(wiki) {
  const notes = [];
