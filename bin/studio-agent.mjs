...
const compileMode = args.mode ?? 'llm'; // 'llm' (default) | 'rule-based'
...
for (const type of only) {
  ...
  console.log(
    `\n[compile] ${type} (schema ${mod.spec.schema_version})${type === 'graphical-abstract' ? ` [mode=${compileMode}]` : ''}`,
  );
  const artifact = await mod.compile(wiki, { mode: compileMode });
