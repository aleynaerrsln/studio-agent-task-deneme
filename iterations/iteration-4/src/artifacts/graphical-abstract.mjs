...
const lintRules = [
  ...
  },
  {
    ...
    check: (a) => {
      ...
      for (const field of spec.numeric_fields ?? []) {
        ...
        for (const v of values) {
          ...
          if (field.extract_numeric_core) {
            for (const core of extractNumericCores(s)) {
              if (!kbNorm.includes(normalizeForTrace(core))) {
          ...
          if (!wholeMatch && coreFailures.length === 0) {
            errors.push(
              `${field.path}: bütün string "${s}" KB'de yok (sayısal çekirdek bulunamadı veya extract devre dışı)`,
            );
          } else if (!wholeMatch && coreFailures.length) {
            errors.push(
              `${field.path}: bütün string "${s}" trace FAIL, ayrıca sayısal çekirdek FAIL [${coreFailures.join(', ')}]`,
            );
          } else if (wholeMatch && coreFailures.length) {
            errors.push(
              `${field.path}: "${s}" — bütün string PASS ama sayısal çekirdek FAIL [${coreFailures.join(', ')}] (compound string KB'de literal var ama içindeki bazı sayılar KB'de yok)`,
            );
          }
        }
      }
      return (
        errors.length === 0 || {
          error: `Numerik alan trace (Level ${spec.trace_level}) başarısız: ${errors.join('; ')}`,
        }
      );
    },
  },
