...
function extractNumericCoresWithContext(s, contextWindow) {
  ...
  while ((m = re.exec(str)) !== null) {
    ...
    const end = Math.min(str.length, m.index + core.length + after);
      ...
      for (const field of spec.numeric_fields ?? []) {
        ...
        for (const v of values) {
          ...
          if (field.extract_numeric_core) {
            const items = extractNumericCoresWithContext(s, field.context_window);
          ...
          if (coreFailures.length > 0) {
            ...
            errors.push(
              `${field.path}: "${s}" — context window mismatch [${contextMismatches.map((c) => `${c.core}@"${c.window}"`).join(', ')}] (core KB'de var ama yanlış bağlamda)`,
            );
          } else if (!wholeMatch && !field.extract_numeric_core) {
            errors.push(`${field.path}: "${s}" — whole string trace FAIL`);
          }
        }
      }
      return (
